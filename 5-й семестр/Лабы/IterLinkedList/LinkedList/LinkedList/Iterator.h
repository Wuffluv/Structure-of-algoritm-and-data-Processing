...
public:
    ...
    virtual Iterator<T>& operator++() = 0; //передвигает итератор на следующий элемент (префиксный инкремент)
    virtual Iterator<T>& operator++(int) = 0; // передвигает итератор на следующий элемент и возвращает старое значение (постфиксный инкремент)
